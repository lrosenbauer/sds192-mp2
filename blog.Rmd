...
```{r}
ggplot(data=hc_stats_2, aes(fill=incumbent, x=ge_winner)) + geom_bar() + scale_fill_pander() + labs(list(title = "How Often do Incumbents Win?", x = "Election Results", y = "Count", fill = "Incumbent Status")) + scale_x_discrete(labels = c("N" = "Lost", "W" = "Won"))
```


```{r}
ggplot(data=hc_stats_2, aes(x = n_contributions, y = total_contribution, color = ge_winner, alpha = 7/10)) + geom_point() + scale_y_continuous(labels = dollar)
